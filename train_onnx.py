...
from obd.models import snn_repvgg,snn_repvgg_sota_onnx,snn_repvgg_onnx
...
def main(args,default_cfg_file = "./default.yaml"):
    ...
    net.detect.stride = torch.tensor([32.]) #torch.tensor([temp_height / x.shape[-2] for x in net(torch.zeros(1,args.in_channels,temp_height,temp_width).cuda())[0]])
    ...
    # Input shape
    dummy_input = torch.randn(2 * 1,1, 192, 256)  # [B, C, H, W] or your actual model input

    # Export
    torch.onnx.export(
            net,
            dummy_input,
            "custom_model_img_civil.onnx",
            input_names=["input"],
            output_names=["output"],
