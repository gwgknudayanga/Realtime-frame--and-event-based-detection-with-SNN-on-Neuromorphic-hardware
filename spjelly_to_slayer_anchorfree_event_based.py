...
def main(args,default_cfg_file = "./default.yaml"):
    ...
    net.load_state_dict(torch.load(args.load)["model"]) #["model_state_dict"])
    ...
    neuron_conv_params = {
                'threshold'     : 1.25, #change the threshold accordingly
                'current_decay' : 1.0,
                'voltage_decay' : 0.25,
                'requires_grad' : False,
    }
    ...
    input = torch.rand(N, C, H, W, T).float().to(device)
    #net_ladl(input)
    ...
    #test_loader = get_dataloader(mode="val", args = args,data = data_info,cfg = cfg,stridee = net.stride)
    test_loader = get_dataloader(args,mode ="val",stridee = net.stride)
    ...
    dest_numpy_data_samples_path = "/home/nrthpc/Udaya_stuff/evCIVIL_direct_train/ultralytic_compatible_ANN_SNN_snn_repvgg/gen1_binary_event_data_numpy"
    ...
    with torch.no_grad():
            ...
            for i,batch in enumerate(val_pbar):
                ...
                """if "crack_100_1370247" in batch["im_file"][0]:  #"crack_100_1005749"
                    print("")
                else:
                    continue"""
                
                """if "17-06-97_12-14-33_122500000_182500000_5.npz" in batch["im_file"][0]:
                    print("")
                else:
                    continue"""
                ...
                #For direct spike event cubes
                batch['img'] = batch['img'].permute(1,0,2,3,4)

                #For voxelgrids/histograms
                ...
                """if count_collected < 250:

                    #for fr_idx,frame in enumerate(temp[:,:,:,:,0]): #for event-frames
                    ...
                    for fr_idx,frame in enumerate(temp):  #for direct spikes
    ...
    output_path = os.path.join("./","sota_direct_evCIVIL_gen1_256_192.net")          #sota_direct_event_gen1_256_192.net")
...
def get_args():
    ...
    #parser.add_argument("-load",type=str,default="/media/atiye/Data/Udaya_Research_stuff/From_server/ultralytic_compatible_ANN_SNN/checkpoints/evCIVIL_lava_img_based/evCIVIL_lava_3.42_after_trained/fused_model/finetune_meanonly_last_129.pt")
    ...
    #parser.add_argument("-load",type=str,default="/media/atiye/Data/Udaya_Research_stuff/ultralytic_compatible_ANN_SNN/checkpoints/evCIVIL_lava_img_based/evCIVIL_lava_3.42_after_trained/fused_model/fused_2.94_best_init.pt")

    #parser.add_argument("-load",type=str,default="/media/atiye/Data/Udaya_Research_stuff/From_server/ultralytic_compatible_ANN_SNN/checkpoints/evCIVIL_lava_event_based/events_7T_frames_distilled/best_evCIVIL_ev_mean_only.pt")
    
    #parser.add_argument("-load",type=str,default="/media/atiye/Data/Udaya_Research_stuff/From_server/ultralytic_compatible_ANN_SNN/checkpoints/gen1_lava_event_based/lava_train/snn_to_lava_frame_distilled/mean_only_finetune_best.pt")
    
    parser.add_argument("-load",type=str,default="/home/nrthpc/Udaya_stuff/From_server_ultralytic_compatible_ANN_SNN_event_based_to_slayer/checkpoints/SOTA_ANCHORFREE/GEN1_lava/DIRECT_SPIKE_TRAIN/best_mean_only_direct_spike.pt")

    #SOTA Anchorfree

    #parser.add_argument("-load",type=str,default="/home/nrthpc/Udaya_stuff/From_server_ultralytic_compatible_ANN_SNN_event_based_to_slayer/checkpoints/SOTA_ANCHORFREE/GEN1_lava/mean_only_fine_tuned_best.pt")

    #parser.add_argument("-load",type=str,default= "/home/nrthpc/Udaya_stuff/From_server_ultralytic_compatible_ANN_SNN_event_based_to_slayer/checkpoints/evCIVIL_lava_event_based/events_7T_frames_distilled/best_evCIVIL_ev_mean_only.pt")   #"/home/nrthpc/Udaya_stuff/Result_Collector/lava_mini_direct/binary_spikes/best_mean_only_evCIVIL.pt")
    #parser.add_argument("-load",type=str,default="/home/nrthpc/Udaya_stuff/From_server_ultralytic_compatible_ANN_SNN_event_based_to_slayer/checkpoints/evCIVIL_lava_event_based/events_7T_frames_distilled/best_evCIVIL_ev_mean_only.pt")
    #parser.add_argument("-load",type=str,default="/home/nrthpc/Udaya_stuff/Result_Collector/lava_mini_direct/gen1/best_initialized_with_snn.pt")

    parser.add_argument('-epoch',  type=int, default=150, help='number of epochs to run')
    

    parser.add_argument('-output_dir', type=str, default='./temp_out_dir', help='directory in which to put log folders')
    ...
    parser.add_argument('-batch_size', type=int, default=1, help='batch_size') #batch = 1 to work "crack_..." switch in the test loop
    ...
    parser.add_argument("-val_img_data_csv",type=str,default="/home/nrthpc/Udaya_stuff/latest_dataset/test_files_event_based.txt")     #"/home/atiye/latest_dataset/test_files_event_based.txt",help="") #night_outdoor_test_files_event_based.txt
    ...
    parser.add_argument('-data_yaml_file',type=str,default="defect.yaml")       #./prophesee_event.yaml")
    parser.add_argument("-input_img_type",type=int, default=3.0, help="")
