...
def quantize_8bit2(x: torch.Tensor, scale = 1 << 6, descale=False):
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            temp_mean = self.running_mean.view(1, -1, 1, 1)
            temp_var = self.running_mean.view(1, -1, 1, 1)
            temp_std = torch.sqrt(temp_var + self.eps)
        ...
        #    -self.weight_exp_bits, self.weight_exp_bits
        #))
        x_mean = self.pre_hook_fx(x_mean)
        
        #t1 = self.pre_hook_fx(self.pre_hook_fx(x/x_std) - self.pre_hook_fx(x_mean/x_std))
        t2 = torch.clamp((x_mean/temp_std),min=-0.98,max=1.1)
        #ratio = t2/(x_mean/x_std)
        #new_val = ratio*t1
        return self.pre_hook_fx(x - self.pre_hook_fx(t2)) #here we already divide x by temp_std"""
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self.track_running_stats and self.training:
            x_mean = torch.mean(x, dim=(0, 2, 3), keepdim=True)
            x_var = torch.var(x,unbiased=False,dim=(0, 2, 3), keepdim=True) #torch.var(x, unbiased=False)


            numel = x.numel() / self.num_features
            with torch.no_grad():
                self.running_mean = (1. - self.momentum) * self.running_mean + self.momentum * x_mean.squeeze()
                self.running_var = (1. - self.momentum) * self.running_var + self.momentum * x_var.squeeze() * numel / (numel + 1) #(1. - self.momentum) * self.running_var + self.momentum * x_var * numel / (numel + 1)
        ...
        print("running var shape ",self.running_var.shape)"""

        """x_std = torch.pow(2., torch.ceil(torch.log2(x_std)).clamp(
            -self.weight_exp_bits, self.weight_exp_bits
        ))"""
        
        x_std = self.pre_hook_fx(x_std)
        x_mean = self.pre_hook_fx(x_mean)
        
        #t1 = self.pre_hook_fx(self.pre_hook_fx(x/x_std) - self.pre_hook_fx(x_mean/x_std))
        t2 = torch.clamp((x_mean/x_std),min=-0.98,max=1.1)
        #ratio = t2/(x_mean/x_std)
        #new_val = ratio*t1
        return self.pre_hook_fx(self.pre_hook_fx(x/x_std) - self.pre_hook_fx(t2)) #self.pre_hook_fx(self.pre_hook_fx(x/x_std) - self.pre_hook_fx(x_mean/x_std))
...
class Temp(nn.Module):
    ...
    def to_lava(self):
        #bn = slayer.neuron.norm.WgtScaleBatchNorm(num_features=self.num_features, momentum=self.momentum, weight_exp_bits=self.weight_exp_bits, eps=self.eps, pre_hook_fx=self.pre_hook_fx)
        bn = slayer.neuron.norm.MeanOnlyBatchNorm(num_features=self.num_features,momentum=self.momentum,pre_hook_fx=self.pre_hook_fx)
        bn.load_state_dict(self.state_dict())
        print(self.state_dict())
        return bn
    ...
    def forward(self, x: torch.Tensor):
        if self.track_running_stats and self.training:
            ...
            with torch.no_grad():
                self.running_mean = (1. - self.momentum) * self.running_mean + self.momentum * x_mean.squeeze()
                #self.running_mean = torch.clamp(self.running_mean, min=-1)
                #self.running_var = (1. - self.momentum) * self.running_var + self.momentum * x_var * numel / (numel + 1)

        else:
            self.running_mean = torch.clamp(self.running_mean, min=-0.98, max=1.1)
            x_mean = self.running_mean.view(1, -1, 1, 1)
            #x_var = self.running_var.view(1, -1, 1, 1)
        ...
        return self.pre_hook_fx(x - self.pre_hook_fx(x_mean))    #/ x_std
...
class CubaRepLIFNode(neuron.BaseNode):
    ...
    def __init__(
            ...
            norm1: BatchNorm2d = None,
            norm2: BatchNorm2d = None,
            detach_reset=False,
        ...
        self.register_memory('current_state1', 0.)
        self.register_memory('current_state2', 0.)
        ...
        if self.norm1 is not None:
            ...
            #if isinstance(self.norm1, BatchNorm2d):
            self.norm1.pre_hook_fx = self.quantize_8bit
            #else:
            #    raise NotImplementedError(self.norm1)
        
        if self.norm2 is not None:

            #if isinstance(self.norm2, BatchNorm2d):
            self.norm2.pre_hook_fx = self.quantize_8bit

            #else:
            # raise NotImplementedError(self.norm2)
    ...
    # computation process
    def state_initialization(self, x1: torch.Tensor, x2: torch.Tensor):
        if isinstance(self.current_state1, float):
            self.current_state1 = torch.zeros_like(x1.data)
        
        if isinstance(self.current_state2, float):
            self.current_state2 = torch.zeros_like(x2.data)

        if isinstance(self.voltage_state, float):
            self.voltage_state = torch.zeros_like(x1.data)
    

    def neuronal_charge(self, x1: torch.Tensor,x2: torch.Tensor):

        if self.requires_grad:
            self.clamp_decay_parameters()

        current1 = LeakyIntegratorStep.apply(
            x1,
            step_quantize(self.current_decay),
            self.current_state1.contiguous(),
            self.s_scale,
        )

        current2 = LeakyIntegratorStep.apply(
            x2,
            step_quantize(self.current_decay),
            self.current_state2.contiguous(),
            self.s_scale,
        )
        ...
        self.current_state1 = current1
        self.current_state2 = current2
    ...
    def single_step_forward(self, x1,x2):
        self.state_initialization(x1,x2)
        self.neuronal_charge(x1,x2)
        spike = self.neuronal_fire()
        self.neuronal_reset(spike)
        return spike

    def multi_step_forward(self, x_seq1: torch.Tensor,x_seq2: torch.Tensor):
        T = x_seq1.shape[0]
        y_seq = []
        if self.store_v_seq:
            v_seq = []
        ...
        for t in range(T):
            y = self.single_step_forward(x_seq1[t],x_seq2[t])
            y_seq.append(y)
            if self.store_v_seq:
...
class CubaLIFNode(neuron.BaseNode):
    ...
    def quantize_8bit(self, x, descale=False):
        ...
        #print("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMM ",self.scale)
        return quantize_8b(x, scale=self.scale, descale=descale)
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        if self.norm is not None and isinstance(self.norm,BatchNorm2d):
            current = x
        else:
...
try:
    ...
    def quantize_8bit(x: torch.Tensor, scale, descale=False):
        ...
        print("scale issssssssssssssssssssssssssss ",scale)
        if descale:
            return step_quantize(x, 2. / scale).clamp(-256. / scale, 255. / scale) * scale
        else:
            return step_quantize(x, 2. / scale).clamp(-256. / scale, 255. / scale)
    ...
    class BlockRepContainer(nn.Module, base.StepModule):
        ...
        def step_mode(self, value: str):
            ...
            if isinstance(self.synapse1, base.StepModule):
                self.synapse1.step_mode = value

            if isinstance(self.synapse2, base.StepModule):
                self.synapse2.step_mode = value

        def __init__(self, synapse1: Union[nn.Conv2d, nn.Linear, nn.AvgPool2d, nn.Flatten], synapse2: Union[nn.Conv2d, nn.Linear, nn.AvgPool2d, nn.Flatten], neu: Optional[CubaRepLIFNode],
                     step_mode: str = 's'):
            super().__init__()

            if isinstance(neu, neuron.LIFNode):
                if neu.v_reset != 0.:
                    raise ValueError('lava only supports for v_reset == 0!')
                if neu.decay_input:
                    raise ValueError('lava only supports for decay_input == False!')
                ...
                assert isinstance(neu, CubaRepLIFNode)

            self.synapse1 = synapse1
            self.synapse2 = synapse2

            self.neuron = neu
            if isinstance(self.synapse1, (nn.Conv2d, nn.Linear)):
                assert self.synapse1.bias is None
            
            if isinstance(self.synapse2, (nn.Conv2d, nn.Linear)):
                assert self.synapse2.bias is None
        ...
        def forward(self, x: torch.Tensor):
            ...
            if isinstance(self.synapse1,nn.Conv2d):
                
                weight = self.neuron.quantize_8bit(self.synapse1.weight)
                # 量化到 2k / self.neuron.scale, k = -128, -127, ..., 127，共有256个取值
                x1 = F.conv2d(x, weight=weight, bias=self.synapse1.bias, stride=self.synapse1.stride,
                                 padding=self.synapse1.padding, dilation=self.synapse1.dilation,
                                 groups=self.synapse1.groups)
            
            if isinstance(self.synapse2,nn.Conv2d):
                weight = self.neuron.quantize_8bit(self.synapse2.weight)
                # 量化到 2k / self.neuron.scale, k = -128, -127, ..., 127，共有256个取值
                x2 = F.conv2d(x, weight=weight, bias=self.synapse2.bias, stride=self.synapse2.stride,
                                 padding=self.synapse2.padding, dilation=self.synapse2.dilation,
                                 groups=self.synapse2.groups)
                
            if self.step_mode == 'm':

                x1 = x1.view([T, N, *x1.shape[1:]])
                x2 = x2.view([T, N, *x2.shape[1:]])
    ...
    class BlockContainer(nn.Module, base.StepModule):
        ...
        def __init__(self, synapse: Union[nn.Conv2d, nn.Linear, nn.AvgPool2d, nn.Flatten], neu: Optional[CubaLIFNode],
                     step_mode: str = 's',batchnorm_version_2 = False,synapse_only: bool = False):
            
            super().__init__()
            self.batchnorm_version_2 = batchnorm_version_2
            self.synapse_only = synapse_only
        ...
        def forward(self, x: torch.Tensor):
            ...
            if isinstance(self.synapse, (nn.Conv2d, nn.Linear)):
                ...
                weight = self.synapse.weight

                if not self.synapse_only:
                    if self.batchnorm_version_2:
                        if self.neuron.norm:
                            #torch.sqrt(x_var + self.eps)1e-05
                            weight = weight / torch.sqrt(self.neuron.norm.running_var + 1e-05)


                    weight = self.neuron.quantize_8bit(weight)
                
                else:
                    weight = quantize_8bit2(weight) #quantize_8bit(weight,descale=False)
