...
class CustomModel(nn.Module):
    ...
    def __init__(self,in_channels = 3,num_classes = 20,T=5):

        super(CustomModel, self).__init__()

        # When exporting , slayer, export upto the end of LavaDetect head_backend
        #Also save the weights for SpikeDFL2_lava

        num_classes = num_classes

        #Instead of Maxpooling we use stride = 2
        #Also instead of decoupled head we use single head for both classification and bounding box regression

        net = torch.nn.ModuleList([

                Lava_GetT(in_channels = in_channels,T=T),   #This makes N,C,H,W to T,N,C,H,W
                ...
                lava_exchange.BlockContainer(
                    layer.Conv2d(16, 16, kernel_size=3, padding=1, stride=1, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(16),detach_reset=True),
                ),


                lava_exchange.BlockContainer(
                    layer.Conv2d(16, 16, kernel_size=3, padding=1, stride=2, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(16),detach_reset=True),
                ),

                lava_exchange.BlockContainer(
                    layer.Conv2d(16,32,kernel_size=3, padding=1, stride=1, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(32),detach_reset=True),
                ),

                lava_exchange.BlockContainer(
                    layer.Conv2d(32,32,kernel_size=3, padding=1, stride=2, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(32),detach_reset=True),
                ),

                lava_exchange.BlockContainer(
                    layer.Conv2d(32,64,kernel_size=3, padding=1, stride=1, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(64),detach_reset=True),
                ),

                lava_exchange.BlockContainer(
                    layer.Conv2d(64,64, kernel_size=3, padding=1, stride=2, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(64),detach_reset=True),
                ),

                lava_exchange.BlockContainer(
                    layer.Conv2d(64,64, kernel_size=3, padding=1, stride=1, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(64),detach_reset=True),
                ),
                ...
                lava_exchange.BlockContainer(
                    layer.Conv2d(in_channels=128,out_channels=24, kernel_size=1, padding=0, stride=1, bias=False),
                    lava_exchange.CubaLIFNode(**neuron_stride_params,norm = lava_exchange.BatchNorm2d(24),detach_reset=True),
                ),

                #Now it is the detection head here onwards.
                #Now out of these 84 channels we need to allocate (4*reg_multiplier) for bbox and 'num_classes' channels to classfication
            ])
