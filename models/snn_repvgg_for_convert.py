...
class QuantizeCustom(nn.Module):
    ...
    class quant2custom(torch.autograd.Function):
        ...
        def forward(ctx, input):
            ...
            #quant_levels = torch.tensor([0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]).to(input.device)
        
            # Clamp the input between 0 and 3
            clamped_input = torch.clamp(input, min=0, max=3.0)

            # Scale the input to map into the quantization set range
            scaled_input = clamped_input / 3.0 * (len(quant_levels) - 1)
...
class mem_update(nn.Module):
    def __init__(self, act=False):
        ...
        self.act = act
        self.qtrick = MultiSpike4() #MultiSpike4()  #QuantizeCustom  # change the max value
...
class SpikeDetect2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = [x]
        shape = x[0].shape  # BCHW  推理：[1，2，64，32，84]  这里必须mean0，否则推理时用到shape会导致报错
        for i in range(self.nl):
            x[i] = self.cv2[i](x[i]) #torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 2)
            x[i] = x[i]  #[2，144，32，684]  #这个地方有时候全是1.之后debug看看
        if self.training:
            return x
...
class SpikeRepVGGBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizee=3, stride=1, padding=1, groups=1):
        ...
        self.act = nn.ReLU() #mem_update()  #nn.SiLU(inplace=True)
