...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max + self.nc, 1,bias=False)) for x in ch)
...
def _yolo(x: torch.tensor,
    ...
    width = (torch.exp(
        x[:, :, :, :, 2:3].clamp(
            max=clamp_max)) * anchor_x[None, :, None, None, None]) / W
    height = (torch.exp(
        x[:, :, :, :, 3:4, :].clamp(
            max=clamp_max)) * anchor_y[None, :, None, None, None]) / H
...
def yolo(x: torch.tensor, anchors: torch.tensor) -> torch.tensor:
        ...
        clamp_max = 5.0
        N, _, _, _, P = x.shape
        return _yolo(x, anchors, clamp_max).reshape([N, -1, P])
...
class AnchorBasedHead(nn.Module):
    ...
    def __init__(self,nc = 20):
        ...
        self.anchors = torch.tensor([[0.2800, 0.2200],
        [0.3800, 0.4800],
        [0.9000, 0.7800]])

        num_output = self.num_anchors * (5 + nc)

        self.head_blocks = nn.Sequential(

            Conv(24,128, kernel_size=3, stride=1, padding=1),
    ...
    def yolo_raw(self,x):
        ...
        return x.reshape(N,self.num_anchors,-1, H, W).permute(0, 1, 3, 4, 2)
    ...
    def forward(self,x):
        ...
        if self.traing:
            return self.yolo_raw(x)

        predictions = self.yolo_raw(x)

        predictions = [predictions]

            #predictions = [torch.sum(prediction,dim=-1).unsqueeze(-1)/args.TSteps for prediction in predictions]
        predictions = [prediction/(T_steps + 0.0) for prediction in predictions]

        predictions = torch.concat([self.yolo(p, a) for (p, a)
                            in zip(predictions, self.anchors)],dim=0) #Since there is no time dimension here
        



############ Anchor-based head Ends ####################
...
class CustomModel(nn.Module):
    ...
    def __init__(self,in_channels = 3,num_classes = 20):
        ...
        self.layers = nn.Sequential(
            Conv(in_channels, 16, kernel_size=3, stride=1, padding=1),
            nn.MaxPool2d(kernel_size=2, stride=2),
            Conv(16, 16, kernel_size=3, stride=1, padding=1),
            Conv(16, 16, kernel_size=3, stride=1, padding=1),
            nn.MaxPool2d(kernel_size=2, stride=2),
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
            Conv(128, 128, kernel_size=3, stride=1, padding=1),
            Conv(128, 24, kernel_size=1, stride=1,padding=0)
        )
        

        self.detect = Detect(nc=num_classes,ch=[24])

    def forward(self, x):
        x = self.layers(x)
        detection_output = self.detect(x)
        return detection_output,None