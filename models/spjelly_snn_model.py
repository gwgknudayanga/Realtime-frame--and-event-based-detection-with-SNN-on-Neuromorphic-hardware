...
from spikingjelly.activation_based import layer,surrogate,lava_exchange
...
class mem_update(nn.Module):
    def __init__(self, act=False):
        ...
        #self.nnn = lava_exchange.CubaLIFNode(**neuron_stride_params,norm = None,detach_reset=False) #neuron.LIFNode(surrogate_function = surrogate.ATan(),detach_reset=True)
...
class SpikeDetect2(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(SpikeConv2(x, c2, 3), SpikeConv2(c2, c2, 3), SpikeConvWithoutBN2(c2, 4 * self.reg_max + nc, 1)) for x in ch)
...
class SpikeConvWithoutBN2(nn.Module): #no lif too
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        self.conv = layer.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.s = s
...
class SpikeConv2(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.conv = layer.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.lif = mem_update()
        self.bn =  nn.BatchNorm2d(c2) #lava_exchange.BatchNorm2d(c2,pre_hook_fx=quantizer) #nn.BatchNorm2d(c2)
        self.s = s
        # self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
...
class CustomModel(nn.Module):
    def __init__(self,in_channels = 3,num_classes = 20,T=5):
        ...
        self.layers = nn.Sequential(
            MS_GetT(in_channels=3,T=T),
            SpikeConv2(in_channels, 16, k=3, s=2, p=1),
            #DownSampling2(k=2, s=2,p=0),
            SpikeConv2(16, 16, k=3, s=1, p=1),
            SpikeConv2(16, 16, k=3, s=2, p=1),
            #DownSampling2(k=2, s=2,p=0),
            SpikeConv2(16, 32, k=3, s=1, p=1),
            SpikeConv2(32, 32, k=3, s=2, p=1),
            #DownSampling2(k=2, s=2,p=0),
            SpikeConv2(32, 64, k=3, s=1, p=1),
            SpikeConv2(64, 64, k=3, s=2, p=1),
            #DownSampling2(k=2, s=2,p=0),
            SpikeConv2(64, 64, k=3, s=1, p=1),
            SpikeConv2(64, 128, k=3, s=2, p=1),
            #DownSampling2(k=2, s=2,p=0),
            SpikeConv2(128, 128, k=3, s=1, p=1),
            SpikeConv2(128, 24, k=1, s=1,p=0)
        )
    ...
    def initialize_model_weights(self) -> None:
        ...
        for i in range(len(self.layers)):
            ...
            if not isinstance(self.layers[i],SpikeConv2):
                continue
            torch.nn.init.kaiming_normal_(self.layers[i].conv.weight.data)

        for i in range(len(self.detect.cv2[0])):

            torch.nn.init.kaiming_normal_(self.detect.cv2[0][i].conv.weight.data)
        
        """for i in range(len(self.detect.cv3[0])):

            torch.nn.init.kaiming_normal_(self.detect.cv3[0][i].conv.weight.data)"""
