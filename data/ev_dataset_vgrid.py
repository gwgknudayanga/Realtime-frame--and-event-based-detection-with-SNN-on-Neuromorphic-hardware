...
import json
from .inference_utils import events_to_voxel_grid_pytorch
from .inference_utils import CropParameters, EventPreprocessor
from .visualize_data import save_images_for_matchscore_calculation,draw_labels_on_image



def draw_yolo_boxes_for_ev_frames(current_frame,current_anns,original_frame_width,
                                  original_frame_height,fr_select_key = "frames",local_name = "",visualize_frame_save_path = "./visualize_data_folder"):

    image_path = save_images_for_matchscore_calculation(visualize_frame_save_path,file_id = local_name, img_array=current_frame, isImgFrame = (fr_select_key == "frames"),isvoxelgrid = False,target_size = (original_frame_width,original_frame_height))
    draw_labels_on_image(image_path,current_anns,format = "yolo")
...
data_visualize_output_path = "/media/atiye/Data/Udaya_Research_stuff/ultralytic_compatible_ANN_SNN/visualize_data"
...
def BBOX2YOLO(anns,img_width,img_height):
...
class TrainValDataset(Dataset):
    ...
    def __init__(
        ...
        self.event_preprocessor = EventPreprocessor(self.target_height,self.target_width,resized_required = False)
        self.crop = CropParameters(self.target_width,self.target_height,num_encoders,self.target_width,self.target_height)
    ...
    def __getitem__(self, index):
        ...
        #event_cube = events_to_voxel_grid_pytorch(events.astype(np.float64),num_bins=3,width=self.param_dict["quantized_w"],height=self.param_dict["quantized_h"],device=torch.device("cpu"))
        ...
        #event_cube = self.event_preprocessor(event_cube)
        #event_cube = self.crop.pad(event_cube)

        

        #print("event_cubeeeeeee  ",event_cube.shape)
        ...
        draw_yolo_boxes_for_ev_frames(current_frame = torch.concatenate((event_cube,torch.zeros(1,256,256)),dim=0).permute(1,2,0),current_anns = torch.concatenate((data_dict["cls"].reshape(-1,1),data_dict["bboxes"].reshape(-1,4)),axis=1),original_frame_width = 256,original_frame_height = 256,fr_select_key = "ev_frames",local_name=index)
