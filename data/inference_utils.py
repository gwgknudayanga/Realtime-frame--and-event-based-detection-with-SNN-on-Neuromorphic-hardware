...
class EventPreprocessor:
    ...
    def __init__(self, resized_height,resized_width,resized_required = False):
        ...
        self.resized_required = resized_required
    ...
    def __call__(self, events):
        ...
        if self.resized_required:
            events = F.interpolate(events, size=(self.resized_height,self.resized_width), mode='bilinear', align_corners=False)
...
class ImageWriter:
    ...
    def __call__(self, img, event_tensor_id, stamp=None, events=None):
        ...
        cv2.imwrite(join(self.output_folder, self.dataset_name,
                         'frame_{:d}.png'.format(int(stamp*1000000))), img)
        if stamp is not None:
            self.timestamps_file.write('{:.6f}\n'.format(stamp))
...
class ImageDisplay:
    ...
    def __call__(self, img, events=None,name = ""):
        ...
        """blank_frame = np.zeros_like(img)  # Create a blank frame of the same size as the image
        cv2.imshow(name, blank_frame)  # Display the blank frame
        cv2.waitKey(1)"""
...
class CropParameters:
    ...
    def __init__(self, width, height, num_encoders,width_crop_size,height_crop_size):
...
def events_to_voxel_grid_pytorch(events, num_bins, width, height, device):
    ...
    with torch.no_grad():
        ...
        with DeviceTimer('Voxel grid voting'):
            ...
            pols[pols == 0] = -1  # polarity should be +1 / -1
            # pols = pols * 0.5  # try to scale the pol
