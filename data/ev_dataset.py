...
class TrainValDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name == "GEN1":
            ...
            #print("labelsssssssssssssssssssss ",labels)
        else:
        ...
        if self.augment:
            ...
            if random.random() <= 0.7:
                ...
                yolo2bbox(labels,304,240)
                events,labels = evaugment_rawevents(events,labels) #this is like affine transformation
                labels[:,[1,3]] = np.clip(labels[:,[1,3]] ,0,304) #since we converted from COCO2YOLO at get_images_and_labels function
                labels[:,[2,4]] = np.clip(labels[:,[2,4]] ,0,240)
                BBOX2YOLO(labels,304,240)
        ...
        dummy_event_cube = make_dvs_frame(events, height=self.param_dict["quantized_h"], width=self.param_dict["quantized_w"], color=False, clip=3,forDisplay = True)
        #print("000000000000000000000000000000000000000000000 ",event_cube.shape)
        dummy_event_cube = torch.from_numpy(dummy_event_cube).float()
        dummy_event_cube, _, _ = evletterbox(dummy_event_cube.unsqueeze(0), target_shape, auto=False,scaleup=True,rect_mode=self.rect)
        ...
        #only when event frames are formed
        #event_cube = event_cube.unsqueeze(0)
        #event_cube = event_cube.repeat(7,1,1,1)
        ...
        data_dict['distill_frame'] = dummy_event_cube#torch.from_numpy(dvs_frame).float().permute(2,0,1)
