...
def distill_loss_feature_maps(s_feature_maps,t_feature_maps,from_snn_distill):
        ...
        if from_snn_distill:
                t_feature_maps_Treduced = [torch.mean(fmap,dim=0)*7.0 for fmap in t_feature_maps]
        else:
                t_feature_maps_Treduced = t_feature_maps 

        return distillation_mse_loss(s_feature_maps_Treduced, t_feature_maps_Treduced)

def distill_loss_channel_attention(ema_s_feature_maps,t_feature_maps,temperature = 1):

        pred_student = F.log_softmax(ema_s_feature_maps / temperature, dim=1)
        pred_teacher = F.softmax(t_feature_maps / temperature, dim=1)
        #log_pred_student = torch.log(pred_student)

        d_loss_channel = F.kl_div(pred_student, pred_teacher, reduction="batchmean")
        d_loss_channel *= temperature**2

        return d_loss_channel

def distill_loss_spatial_attention(ema_s_feature_maps,t_feature_maps):
       
       student_attention = F.normalize(ema_s_feature_maps, p=2, dim=1)  # L2 Normalization
       teacher_attention = F.normalize(t_feature_maps, p=2, dim=1)
       # Compute cosine similarity loss (1 - cosine similarity)
       d_loss_spatial = 1 - F.cosine_similarity(student_attention, teacher_attention, dim=1).mean()
       return d_loss_spatial
