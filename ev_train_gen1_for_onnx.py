...
from obd.models import lava_snn_model2_sota,lava_snn_model2,snn_repvgg_sota_onnx,lava_snn_model2_feedforward,lava_snn_model2_feedforward_mean #impot CustomModel
from obd.models import snn_repvgg,snn_repvgg_onnx
...
class AnchorFreePredDecoder():
    ...
    def __init__(self,reg_max,num_classes,stride,device):
        ...
        self.dfl = snn_repvgg_sota_onnx.SpikeDFL2(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
def main(args,default_cfg_file = "./default.yaml"):
    ...
    net = snn_repvgg_sota_onnx.CustomModel(in_channels = args.in_channels ,num_classes = data_info['nc'] ,for_distillation = False,T=1)
    ...
    net(torch.zeros(1,args.in_channels,temp_height,temp_width).cuda())

    #net.detect.stride 
    net.detect.stride = torch.tensor([32.])
    ...
    # Make sure export flags are set

    net.cpu()

    net.detect.export = True
    net.detect.format = 'onnx'

    # Input shape
    dummy_input = torch.randn(2 * 1, 2, 192, 256)  # [B, C, H, W] or your actual model input

    # Export
    torch.onnx.export(
            net,
            dummy_input,
            "sota_ev_model_civil.onnx",
            input_names=["input"],
            output_names=["output"],
    ...
    anchorfree_decoder  = AnchorFreePredDecoder(reg_max=5,num_classes=data_info['nc'],stride=net.detect.stride,device=device)
    ...
    for epoch in range(current_epoch,args.epoch):
        ...
        with torch.no_grad():
            ...
            for i,batch in enumerate(val_pbar):
                ...
                preds = anchorfree_decoder.decode_predictions(x=preds,shape=preds.shape)
...
def get_args():
    ...
    parser.add_argument('-epoch',  type=int, default=400, help='number of epochs to run')
    ...
    #parser.add_argument("-load",default="/home/nrthpc/Udaya_stuff/Result_Collector/lava_mini_direct/gen1/last.pt",type=str)
    ...
    #parser.add_argument("-load",type=str,default="/home/nrthpc/Udaya_stuff/Result_Collector/snn_mini/gen1/best.pt")
    #parser.add_argument("-load",type=str,default="/home/nrthpc/Udaya_stuff/Result_Collector/SOTA_Anchorfree_ann/gen1/best.pt")
    ...
    #parser.add_argument("-load",default="/home/nrthpc/Udaya_stuff/Result_Collector/SOTA_Anchorfree_ann/gen1/best.pt")
    #parser.add_argument("-load",default="/home/nrthpc/Udaya_stuff/From_server_ultralytic_compatible_ANN_SNN_event_based_to_slayer/checkpoints/SOTA_ANCHORFREE/GEN1_lava/gen1_lava_direct_sota_anchorfree.pt")
    ...
    parser.add_argument("-val_img_data_csv",type=str,default="/home/nrthpc/Udaya_stuff/latest_dataset/test_files_event_based.txt",help="")
    ...
    parser.add_argument("-val_img_data_csv",type=str,default="/home/nrthpc/Yan_SC_Stuff/prophesee_processed_dataset/gen1_val_a.csv",help="")
    parser.add_argument("-TSteps",type=int,default=7,help="Tsteps for the event cube bins")
